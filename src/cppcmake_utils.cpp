...
bool CppCmake::CppCmakeMain::RebuildManifest(const char* input_file, std::string* err, Status* status) {
...
void CppCmake::CppCmakeMain::ParsePreviousElapsedTimes() {
  for (Edge* edge : state_.edges_) {
    for (Node* out : edge->outputs_) {
      ...
      edge->prev_elapsed_time_millis = log_entry->end_time - log_entry->start_time;
...
bool CppCmake::CppCmakeMain::CollectTargetsFromArgs(int argc, char* argv[], std::vector<Node*>* targets,
                                                    std::string* err) {
  if (argc == 0) {
...
int CppCmake::CppCmakeMain::ToolQuery(const Options* options, int argc, char* argv[]) {
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    printf("  outputs:\n");
    for (std::vector<Edge*>::const_iterator edge = node->out_edges().begin(); edge != node->out_edges().end(); ++edge) {
      for (std::vector<Node*>::iterator out = (*edge)->outputs_.begin(); out != (*edge)->outputs_.end(); ++out) {
        printf("    %s\n", (*out)->path().c_str());
      }
    }
...
int CppCmake::ToolTargetsSourceList(State* state) {
  for (std::vector<Edge*>::iterator e = state->edges_.begin(); e != state->edges_.end(); ++e) {
    for (std::vector<Node*>::iterator inps = (*e)->inputs_.begin(); inps != (*e)->inputs_.end(); ++inps) {
      if (!(*inps)->in_edge())
        printf("%s\n", (*inps)->path().c_str());
    }
  }
...
int CppCmake::ToolTargetsList(State* state, const std::string& rule_name) {
  ...
  for (std::vector<Edge*>::iterator e = state->edges_.begin(); e != state->edges_.end(); ++e) {
    if ((*e)->rule_->name() == rule_name) {
      for (std::vector<Node*>::iterator out_node = (*e)->outputs_.begin(); out_node != (*e)->outputs_.end();
           ++out_node) {
...
int CppCmake::ToolTargetsList(State* state) {
  for (std::vector<Edge*>::iterator e = state->edges_.begin(); e != state->edges_.end(); ++e) {
    for (std::vector<Node*>::iterator out_node = (*e)->outputs_.begin(); out_node != (*e)->outputs_.end(); ++out_node) {
...
int CppCmake::CppCmakeMain::ToolDeps(const Options* options, int argc, char** argv) {
  std::vector<Node*> nodes;
  if (argc == 0) {
    for (std::vector<Node*>::const_iterator ni = deps_log_.nodes().begin(); ni != deps_log_.nodes().end(); ++ni) {
...
int CppCmake::CppCmakeMain::ToolRules(const Options* options, int argc, char* argv[]) {
  ...
  while ((opt = getopt(argc, argv, const_cast<char*>("hd"))) != -1) {
    switch (opt) {
      ...
      default:
        printf(
            "usage: cppcmake -t rules [options]\n"
            "\n"
            "options:\n"
...
int CppCmake::CppCmakeMain::ToolCommands(const Options* options, int argc, char* argv[]) {
  ...
  while ((opt = getopt(argc, argv, const_cast<char*>("hs"))) != -1) {
    switch (opt) {
      ...
      default:
        printf(
            "usage: cppcmake -t commands [options] [targets]\n"
            "\n"
            "options:\n"
...
void CppCmake::CollectInputs(Edge* edge, std::set<Edge*>* seen, std::vector<std::string>* result) {
  if (!edge)
    return;
  if (!seen->insert(edge).second)
    return;
...
int CppCmake::CppCmakeMain::ToolClean(const Options* options, int argc, char* argv[]) {
  ...
  while ((opt = getopt(argc, argv, const_cast<char*>("hgr"))) != -1) {
    switch (opt) {
      ...
      default:
        printf(
            "usage: cppcmake -t clean [options] [targets]\n"
            "\n"
            "options:\n"
...
std::string CppCmake::EvaluateCommandWithRspfile(const Edge* edge, const EvaluateCommandMode mode) {
...
void CppCmake::printCompdb(const char* const directory, const Edge* const edge, const EvaluateCommandMode eval_mode) {
  printf("\n  {\n    \"directory\": \"");
...
int CppCmake::CppCmakeMain::ToolCompilationDatabase(const Options* options, int argc, char* argv[]) {
  ...
  putchar('[');
  for (std::vector<Edge*>::iterator e = state_.edges_.begin(); e != state_.edges_.end(); ++e) {
    if ((*e)->inputs_.empty())
      continue;
    if (argc == 0) {
      if (!first) {
...
const CppCmake::Tool* CppCmake::ChooseTool(const std::string& tool_name) {
  static const Tool kTools[] = {
      {"browse", "browse dependency graph in a web browser", Tool::RUN_AFTER_LOAD, &CppCmake::CppCmakeMain::ToolBrowse},
#ifdef _WIN32
      {"msvc", "build helper for MSVC cl.exe (DEPRECATED)", Tool::RUN_AFTER_FLAGS, &CppCmake::CppCmakeMain::ToolMSVC},
#endif
      {"clean", "clean built files", Tool::RUN_AFTER_LOAD, &CppCmake::CppCmakeMain::ToolClean},
      {"commands", "list all commands required to rebuild given targets", Tool::RUN_AFTER_LOAD,
       &CppCmake::CppCmakeMain::ToolCommands},
      {"inputs", "list all inputs required to rebuild given targets", Tool::RUN_AFTER_LOAD,
       &CppCmake::CppCmakeMain::ToolInputs},
      {"deps", "show dependencies stored in the deps log", Tool::RUN_AFTER_LOGS, &CppCmake::CppCmakeMain::ToolDeps},
      {"missingdeps", "check deps log dependencies on generated files", Tool::RUN_AFTER_LOGS,
       &CppCmake::CppCmakeMain::ToolMissingDeps},
      {"graph", "output graphviz dot file for targets", Tool::RUN_AFTER_LOAD, &CppCmake::CppCmakeMain::ToolGraph},
      {"query", "show inputs/outputs for a path", Tool::RUN_AFTER_LOGS, &CppCmake::CppCmakeMain::ToolQuery},
      {"targets", "list targets by their rule or depth in the DAG", Tool::RUN_AFTER_LOAD,
       &CppCmake::CppCmakeMain::ToolTargets},
      {"compdb", "dump JSON compilation database to stdout", Tool::RUN_AFTER_LOAD,
       &CppCmake::CppCmakeMain::ToolCompilationDatabase},
      {"recompact", "recompacts cppcmake-internal data structures", Tool::RUN_AFTER_LOAD,
       &CppCmake::CppCmakeMain::ToolRecompact},
      {"restat", "restats all outputs in the build log", Tool::RUN_AFTER_FLAGS, &CppCmake::CppCmakeMain::ToolRestat},
      {"rules", "list all rules", Tool::RUN_AFTER_LOAD, &CppCmake::CppCmakeMain::ToolRules},
      {"cleandead", "clean built files that are no longer produced by the manifest", Tool::RUN_AFTER_LOGS,
       &CppCmake::CppCmakeMain::ToolCleanDead},
      {"urtle", NULL, Tool::RUN_AFTER_FLAGS, &CppCmake::CppCmakeMain::ToolUrtle},
#ifdef _WIN32
      {"wincodepage", "print the Windows code page used by cppcmake", Tool::RUN_AFTER_FLAGS,
       &CppCmake::CppCmakeMain::ToolWinCodePage},
#endif
...
bool CppCmake::WarningEnable(const std::string& name, Options* options) {
  if (name == "list") {
    ...
    const char* suggestion = SpellcheckString(name.c_str(), "phonycycle=err", "phonycycle=warn", nullptr);
    if (suggestion) {
...
int CppCmake::ReadFlags(int* argc, char*** argv, Options* options, BuildConfig* config) {
