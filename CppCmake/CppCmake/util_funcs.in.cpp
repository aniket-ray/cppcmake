/* Generated by re2c 3.1 on Sat May 25 13:15:55 2024 */
...
bool CppCmake::tokenizer(const std::string& string_, EvalString* let_value, std::string* err) {
...
yy4:
	{
       let_value->AddText(StringPiece(token));
     }
yy5:
	yych = *++YYCURSOR;
	switch (yych) {
...
yy11:
	{
       eval->AddSpecial(StringPiece(token.substr(1));
       continue;
     }
yy12:
	yych = *++YYCURSOR;
...
yy13:
	++YYCURSOR;
	{
        eval->AddSpecial(StringPiece(token.substr(2, token.size()-3)));
        continue;
     }
}
...
std::string CppCmake::extract_references(const std::string& ref) {}
