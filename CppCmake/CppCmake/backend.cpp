...
std::string CppCmake::Make::generate_string_() {
  ...
  for (auto r : this->rules_) {
    ...
    if (!r.msvc_deps_prefix.empty()) {
      out += "msvc_deps_prefix = " + r.msvc_deps_prefix + "\n";
      out += "  ";
    }

    if (!r.generator.empty()) {
      out += "generator = " + r.generator + "\n";
      out += "  ";
    }
    ...
    if (!r.rspfile.empty()) {
      out += "rspfile = " + r.rspfile + "\n";
      out += "  ";
    }

    if (!r.rspfile_content.empty()) {
      out += "rspfile_content = " + r.rspfile_content + "\n";
      out += "  ";
    }
